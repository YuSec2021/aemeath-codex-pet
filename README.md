...
- 调整 row 1 `running-right` 的动作帧顺序，使奔跑步态和循环衔接更加顺畅。
- 以已通过检查的 `running-right` 为动作及时序基准，逐帧水平镜像生成 row 2 `running-left`；保持帧序不反转，让左右奔跑拥有一致的交替步态。
- 重新组装并检查完整 v2 图集；人物边缘、动作方向和循环闭合均通过视觉复核，最终校验为 0 错误、0 警告。
